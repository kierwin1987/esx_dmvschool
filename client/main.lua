...
function zaplaceni_teorie()
    ESX.UI.Menu.CloseAll()
    
    local elements = {
        {label = 'Platba hotove', value = 'hotove'},
        {label = 'Platba kartou', value = 'kartou'}
    }

    ESX.UI.Menu.CloseAll()

    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'zaplaceni',
        {
            title    = 'Vyber zaplacení',
            align    = 'right',
            elements = elements
        },
        ...
        function(data, menu)
            ...
            if data.current.value == 'hotove' then
                menu.close()
                TriggerServerEvent('esx_dmvschool:zaplacenihotoveteorie')
            end
            
            if data.current.value == 'kartou' then
                menu.close()
                TriggerServerEvent('esx_dmvschool:zaplacenikartouteorie')
            end
        ...
        function(data, menu)
            ...
            CurrentAction     = 'zaplaceni'
        end)
end

function zaplaceni_prakticke(type)
	
	local prakticketyp = type
    ESX.UI.Menu.CloseAll()
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            if data.current.value == 'hotove' then
                menu.close()
                TriggerServerEvent('esx_dmvschool:zaplacenihotoveprakticka', prakticketyp)
            end
            
            if data.current.value == 'kartou' then
                menu.close()
                TriggerServerEvent('esx_dmvschool:zaplacenikartouprakticka', prakticketyp)
            end
...
function StartDriveTest(typ)
	ESX.Game.SpawnVehicle(Config.VehicleModels[typ], Config.Zones.VehicleSpawnPoint.Pos, Config.Zones.VehicleSpawnPoint.Pos.h, function(vehicle)
		CurrentTest       = 'drive'
		CurrentTestType   = typ
		CurrentCheckPoint = 0
		LastCheckPoint    = -1
...
RegisterNetEvent('esx_dmvschool:zaplacenateorie')
AddEventHandler('esx_dmvschool:zaplacenateorie', function()
	StartTheoryTest()
end)

RegisterNetEvent('esx_dmvschool:zaplacenaprakticka')
AddEventHandler('esx_dmvschool:zaplacenaprakticka', function(typ)
	StartDriveTest(typ)
end)
...
--			StartTheoryTest()
		elseif data.current.value == 'drive_test' then
			zaplaceni_prakticke(data.current.type)
